    ...
    <ResearchLayout activeDashboard={dashboard} onNavigate={setDashboard}>
      <DashboardHeader
        ...
        actions={
          <button
            ...
            style={{ padding: '0.5rem 1.25rem', borderRadius: '8px', background: '#6366f1', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem' }}
          >
            + New QR Campaign
          </button>
        }
      ...
      {analytics && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(170px, 1fr))', gap: '0.75rem', marginBottom: '1.5rem' }}>
          <StatCard label="Total Campaigns" value={campaigns.length} color="#6366f1" />
          <StatCard label="Total Scans" value={campaigns.reduce((s, c) => s + c.scan_count, 0)} color="#22c55e" />
      ...
      {campaigns.length > 0 && analytics && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
          {analytics.sessionCompletionByCampaign.length > 0 && (
            <div style={{ background: '#12121a', border: '1px solid #1e1e2e', borderRadius: '12px', padding: '1rem' }}>
              <BarChart data={analytics.sessionCompletionByCampaign.map(d => ({ label: d.campaign, value: d.rate, color: '#22c55e' }))} title="Completion Rate %" />
            </div>
          )}
          {analytics.avgFocusByCampaign.length > 0 && (
            <div style={{ background: '#12121a', border: '1px solid #1e1e2e', borderRadius: '12px', padding: '1rem' }}>
              <BarChart data={analytics.avgFocusByCampaign.map(d => ({ label: d.campaign, value: d.avgFocus, color: '#6366f1' }))} title="Avg Focus Score" />
            </div>
          )}
        </div>
      )}
              ...
              {campaigns.map(c => {
                ...
                return (
                  <tr
                    ...
                    style={{ cursor: 'pointer', background: selectedCampaign?.id === c.id ? '#1a1a2e' : 'transparent', transition: 'background 0.1s' }}
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#16162a'; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = selectedCampaign?.id === c.id ? '#1a1a2e' : 'transparent'; }}
                  >
                    <td style={ROW_STYLE}>{c.name}</td>
                    <td style={ROW_STYLE}>{c.location_type ?? '-'}</td>
                    <td style={ROW_STYLE}>{[c.city, c.country].filter(Boolean).join(', ') || '-'}</td>
                    <td style={{ ...ROW_STYLE, fontVariantNumeric: 'tabular-nums' }}>{c.scan_count}</td>
                    <td style={{ ...ROW_STYLE, fontVariantNumeric: 'tabular-nums' }}>{c.game_complete_count}</td>
                    <td style={{ ...ROW_STYLE, fontVariantNumeric: 'tabular-nums' }}>{c.registration_count}</td>
                    <td style={{ ...ROW_STYLE, color: parseFloat(conversion) >= 20 ? '#22c55e' : parseFloat(conversion) >= 10 ? '#f59e0b' : '#888' }}>{conversion}%</td>
                    <td style={ROW_STYLE}>{c.qr_count}</td>
                    <td style={{ ...ROW_STYLE, color: c.is_active ? '#22c55e' : '#666' }}>{c.is_active ? 'Active' : 'Inactive'}</td>
                  </tr>
                );
              })}
              {campaigns.length === 0 && (
                <tr><td colSpan={9} style={{ padding: '2rem', textAlign: 'center', color: '#555' }}>No campaigns yet. Create your first one!</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function CampaignDetail({ campaign: c }: { campaign: CampaignRow }) {
  ...
  useEffect(() => {
    QRCodeLib.toDataURL(url, { width: 200, margin: 2, color: { dark: '#f0f0f0', light: '#0a0a0f' } }).then(setQrImage);
  }, [url]);
...
function CampaignWizard({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const handleCreate = async () => {
    ...
    const qrData = await ds.createQRCode({
  ...
  const overlayStyle: React.CSSProperties = { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' };
  const modalStyle: React.CSSProperties = { background: '#12121a', border: '1px solid #1e1e2e', borderRadius: '16px', padding: '1.5rem', width: '90%', maxWidth: '520px', maxHeight: '90vh', overflowY: 'auto', color: '#f0f0f0' };

  if (result) {
    return (
      <div style={overlayStyle} onClick={onClose}>
        <div style={modalStyle} onClick={e => e.stopPropagation()}>
          <h3 style={{ margin: '0 0 1rem', fontSize: '1.1rem' }}>Campaign Created!</h3>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', marginBottom: '1rem' }}>
            <button onClick={() => downloadQR('png')} style={{ padding: '0.5rem 1rem', borderRadius: '8px', background: '#6366f1', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Download PNG</button>
            <button onClick={() => downloadQR('svg')} style={{ padding: '0.5rem 1rem', borderRadius: '8px', background: '#1e1e2e', color: '#ccc', border: '1px solid #333', cursor: 'pointer' }}>Download SVG</button>
          </div>
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={e => e.stopPropagation()}>
        ...
        {step === 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <div>
              ...
              <input style={inputStyle} value={country} onChange={e => setCountry(e.target.value)} placeholder="e.g. Saudi Arabia" />
            </div>
            <div>
              <label style={labelStyle}>State / Province</label>
              <input style={inputStyle} value={stateName} onChange={e => setStateName(e.target.value)} placeholder="e.g. Riyadh" />
            </div>
            <div>
              <label style={labelStyle}>City</label>
              <input style={inputStyle} value={city} onChange={e => setCity(e.target.value)} placeholder="e.g. Riyadh" />
            </div>
            <div>
              <label style={labelStyle}>District (optional)</label>
              <input style={inputStyle} value={district} onChange={e => setDistrict(e.target.value)} placeholder="e.g. Gate A, 2nd Floor" />
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div>
            ...
            <textarea
              ...
              placeholder="e.g. Near entrance, by the cashier, Poster #3"
            />
            <div style={{ marginTop: '0.75rem', padding: '0.75rem', background: '#1e1e2e', borderRadius: '8px', fontSize: '0.8rem', color: '#888' }}>
              <p style={{ margin: '0 0 0.3rem', color: '#f0f0f0', fontWeight: 600 }}>Review</p>
              <p style={{ margin: 0 }}><strong style={{ color: '#ccc' }}>Name:</strong> {name}</p>
              <p style={{ margin: 0 }}><strong style={{ color: '#ccc' }}>Type:</strong> {locationType}</p>
              <p style={{ margin: 0 }}><strong style={{ color: '#ccc' }}>Location:</strong> {[district, city, stateName, country].filter(Boolean).join(', ') || '-'}</p>
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1.25rem' }}>
          {step > 1 && (
            <button onClick={() => setStep(s => s - 1)} style={{ flex: 1, padding: '0.6rem', borderRadius: '8px', background: '#1e1e2e', color: '#888', border: '1px solid #333', cursor: 'pointer' }}>Back</button>
          )}
          {step < 4 ? (
            <button
